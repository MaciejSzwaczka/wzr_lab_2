...
ObiektRuchomy::ObiektRuchomy(Teren *teren,float x,float y, float z)
{
  ObiektRuchomy();
  ...
  this->teren = teren;
  wPol = teren->PunktMax(Wektor3(x,y,z)) + Wektor3(x,y,z)*(przeswit+wysokosc/2 + 20);
  ...
  wV_kat = Wektor3(x,y,z)*0;//40;  // pocz¹tkowa prêdkoœæ k¹towa (w celach testowych)
  promien = sqrt(dlugosc*dlugosc + szerokosc*szerokosc + wysokosc*wysokosc)/2/1.15;

	// obrót obiektu o k¹t 30 stopni wzglêdem osi y:
	kwaternion qObr = AsixToQuat(Wektor3(x,y,z),0.1*PI/180.0);
